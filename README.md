...
- [TrustBoost PII Sanitizer](https://github.com/teodorofodocrispin-cmyk/TrustBoost-PII-Sanitizer) - Context-aware PII sanitization for autonomous AI agent pipelines. Redacts emails, national IDs, API keys, and financial data before text reaches LLMs. Every paid sanitization anchored on Solana via Helius — verifiable at `/verify/{anchor_tx}`. 8 languages including LATAM (RFC, CPF, CUIT) and EU identifiers. x402 compatible for autonomous USDC payment. EU AI Act compliant. (🏷️ `Python` `Privacy` `API` `[GDPR]` `[EU-AI-Act]` `[Solana]` `[MCP]`).
- [TrustBoost PII Sanitizer](https://github.com/teodorofodocrispin-cmyk/TrustBoost-PII-Sanitizer) - Context-aware PII sanitization for autonomous AI agent pipelines. Redacts emails, national IDs, API keys, and financial data before text reaches LLMs. Every paid sanitization anchored on Solana via Helius — verifiable at `/verify/{anchor_tx}`. 8 languages including LATAM (RFC, CPF, CUIT) and EU identifiers. x402 compatible for autonomous USDC payment. EU AI Act compliant. (🏷️ `Python` `Privacy` `API` `[GDPR]` `[EU-AI-Act]` `[Solana]` `[MCP]`).
